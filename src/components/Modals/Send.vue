  ...
  >
    <v-card>
      ...
      <v-container v-else>
        <v-row>
          <v-col cols="12">
            <div class="grey--text font-weight-bold text-caption">
              {{ $t('From') }}
            </div>
    ...
    show: {
      ...
      async set () {
        ...
        if (response === this.input) {
          ...
          const result = await this.$sdk.send(keys, this.to, this.amount, this.memo, this.contract.name).catch((error) => { this.error = error});
